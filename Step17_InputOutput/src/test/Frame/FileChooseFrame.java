...
public class FileChooseFrame extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == selectBtn) {
			selectGeneral();
		}else if(obj==selectBtn2){//.txt 파일선택버튼을 눌렀을때
			selectText();
		}
	...
	public void selectGeneral() {
		//c:/myFolder 에서 시작되는 JFileChooser 객체생성
		JFileChooser fc=new JFileChooser("c:/myFolder");
		//파일을 선택해서 열수 있는 다이얼로그 띄우고 어떤 작업을 했는지 결과를 int로 받기
		int result=fc.showOpenDialog(this);
		
		if(result == JFileChooser.APPROVE_OPTION) {//파일을 선택하고 확인을 눌렀을때
			//선택한 파일을 access 할 수 있는 File 객체 의 참조값 얻어오기
			...
			JOptionPane.showMessageDialog(this, "취소를 눌렀네요?");
	...
	public void selectText() {
		JFileChooser fc=new JFileChooser("c:/myFolder");
		//확장자가 txt 인 파일만 선택할 수 있도록 필터 설정
		FileNameExtensionFilter filter=
				new FileNameExtensionFilter("텍스트파일","txt");
		fc.setFileFilter(filter);
		fc.showOpenDialog(this);
		//선택된 텍스트 문서에 저장된 문자열을 읽어서 콘솔창에 모두 출력 할 수 있습니까?
		int result=fc.showOpenDialog(this);
